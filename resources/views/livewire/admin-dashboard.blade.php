<style>
    ...
    @keyframes glowPulseAmber  { 0%,100%{box-shadow:0 0 0 0 rgba(245,158,11,0);}   50%{box-shadow:0 0 20px 6px rgba(245,158,11,0.25);} }
    ...
    .glow-amber  { animation: glowPulseAmber  3s ease-in-out infinite; }
    ...
    .check-bounce { animation: checkBounce 0.35s ease both; }
    ...
    .banner-slide { animation: bannerSlide 0.4s ease both; }
...
<div
    x-data="{
        clock: '{{ date('h:i:s A') }}',
        init() { setInterval(() => {
            this.clock = new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
        }, 1000); }
    }"
...
<div class="grid grid-cols-12 gap-4 p-5 pb-0">
        ...
        <div>
            <div class="flex items-center gap-2 mb-1">
                @if($systemReady)
                    ...
                    <span class="text-[11px] tracking-[0.25em] text-emerald-600 dark:text-emerald-400 uppercase font-semibold">
                        System Live — {{ $systemStatus['current_semester'] ?? '—' }}
                    </span>
                @else
                    <span class="inline-block w-2 h-2 rounded-full bg-amber-400 shadow-[0_0_8px_#fbbf24] animate-pulse"></span>
                    <span class="text-[11px] tracking-[0.25em] text-amber-600 dark:text-amber-400 uppercase font-semibold">
                        Setup Required — {{ $systemStatus['current_semester'] ?? '—' }}
                    </span>
                @endif
            </div>
            ...
            <p class="text-[12px] text-slate-500 mt-1">
                {{ date('l, F d, Y') }}
                &nbsp;·&nbsp;
                <span class="text-slate-600 dark:text-slate-400">System Administrator</span>
            </p>
        </div>
    ...
    @php
        $vitals = [
            ...
            ['label'=>'Faculty',        'value'=>$stats['total_faculty'],  'color'=>'violet',
             'icon'=>'<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"/></svg>'],
            ['label'=>'Rooms',          'value'=>$stats['total_rooms'],    'color'=>'cyan',
            ...
            ['label'=>'Total Schedules','value'=>$stats['total_subjects'] ?? $stats['total_schedules'], 'color'=>'emerald',
...
{{-- ═══════════════════════════════════════════════════
     ...
     Shown to Admin/Registrar when system is NOT ready.
     Sticks right under the header — impossible to miss.
═══════════════════════════════════════════════════════ --}}
@if(! $systemReady)
<div class="px-5 pt-4 banner-slide">
    <div class="relative overflow-hidden rounded-2xl border border-amber-300 dark:border-amber-500/40
                bg-gradient-to-br from-amber-50 via-orange-50 to-amber-50
                dark:from-amber-500/[0.06] dark:via-orange-500/[0.04] dark:to-amber-500/[0.06]
                glow-amber shadow-sm">

        {{-- Decorative background glow orb --}}
        <div class="pointer-events-none absolute -top-10 -right-10 w-48 h-48 rounded-full
                    bg-amber-400/10 dark:bg-amber-400/5 blur-2xl"></div>
        ...
        <div class="relative z-10 p-5">
            ...
            <div class="flex items-start justify-between gap-4 mb-5">
                <div class="flex items-start gap-3">
                    {{-- Icon --}}
                    <div class="flex-shrink-0 w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-500/15
                    ...
                    <div>
                        <h2 class="text-[15px] font-bold text-amber-800 dark:text-amber-300 tracking-tight">
                            New Semester — Setup Required
                        </h2>
                        <p class="text-[12px] text-amber-700/80 dark:text-amber-400/70 mt-0.5 leading-snug">
                            Complete the checklist below, then mark the system as ready to unlock full access for Dean and OIC roles.
                        </p>
                    </div>
                </div>
                ...
                <div class="flex-shrink-0 flex flex-col items-end gap-2">
                    ...
                    <span class="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold">All steps complete ✓</span>
                    @else
                    <button disabled
                            class="flex items-center gap-2 px-4 py-2 rounded-xl
                                   bg-slate-200 dark:bg-white/10
                                   text-slate-400 dark:text-slate-500 text-[12px] font-bold uppercase tracking-wider
                    ...
                    @php $remaining = collect($setupChecklist)->where('done', false)->count(); @endphp
                    <span class="text-[10px] text-amber-600 dark:text-amber-400">{{ $remaining }} step{{ $remaining !== 1 ? 's' : '' }} remaining</span>
                    @endif
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                ...
                <div class="flex items-start gap-2.5 p-3 rounded-xl
                            {{ $item['done']
                                ? 'bg-emerald-50 dark:bg-emerald-500/[0.06] border border-emerald-200 dark:border-emerald-500/25'
                                : 'bg-white/60 dark:bg-white/[0.03] border border-amber-200/60 dark:border-amber-500/20' }}
                            transition-colors duration-300">

                    {{-- Check / pending icon --}}
                    <div class="flex-shrink-0 mt-0.5">
                        @if($item['done'])
                        <div class="check-bounce w-5 h-5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]
                                    flex items-center justify-center">
                        ...
                        @else
                        <div class="w-5 h-5 rounded-full border-2 border-amber-300 dark:border-amber-500/50
                                    bg-amber-100 dark:bg-amber-500/10 flex items-center justify-center">
                    ...
                    <div class="min-w-0 flex-1">
                        <p class="text-[12px] font-bold leading-snug
                                  {{ $item['done'] ? 'text-emerald-700 dark:text-emerald-400' : 'text-amber-800 dark:text-amber-300' }}">
                            {{ $item['label'] }}
                        </p>
                        <p class="text-[10px] leading-snug mt-0.5
                                  {{ $item['done'] ? 'text-emerald-600/70 dark:text-emerald-500/70' : 'text-amber-700/70 dark:text-amber-400/60' }}">
                            {{ $item['description'] }}
                        </p>
                    </div>
                </div>
            ...
            <div class="mt-4 pt-4 border-t border-amber-200/60 dark:border-amber-500/20 flex items-center justify-between">
...
<div class="px-5 pt-4 banner-slide">
    <div class="flex items-center justify-between px-5 py-3 rounded-2xl
                bg-emerald-50 dark:bg-emerald-500/[0.05]
                border border-emerald-200 dark:border-emerald-500/25">
        <div class="flex items-center gap-3">
            <span class="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_8px_#34d399] animate-pulse flex-shrink-0"></span>
            <div>
                <span class="text-[13px] font-bold text-emerald-700 dark:text-emerald-400">
                    System is Live
                </span>
                <span class="text-[12px] text-slate-500 dark:text-slate-400 ml-2">—</span>
                <span class="text-[12px] text-slate-500 dark:text-slate-400 ml-1">
                    {{ $systemStatus['current_semester'] ?? '—' }}
                </span>
                @if(! empty($systemReadyMeta['ready_at']))
                <span class="text-[11px] text-slate-400 ml-2">
                    · Ready since {{ \Carbon\Carbon::parse($systemReadyMeta['ready_at'])->format('M d, h:i A') }}
                </span>
                @endif
            </div>
        </div>
...
@php
    $wf = $workflowCounts;
    $wfCards = [
        ['label'=>'Draft',        'count'=>$wf['draft'] ?? 0,              'desc'=>'Initial schedule entries',       'color'=>'slate',
         'glow'=>'', 'dot'=>'bg-slate-400',
         'badge'=>'bg-slate-100 dark:bg-slate-500/10 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-500/20',
         'border'=>'border-slate-200 dark:border-white/10'],
        ['label'=>'Generated',    'count'=>$wf['partial'] ?? 0,            'desc'=>'Auto-generated by Registrar',    'color'=>'blue',
        ...
        ['label'=>'Under Review',  'count'=>$wf['faculty_assigned'] ?? 0,   'desc'=>'Faculty assigned & dept review', 'color'=>'yellow',
        ...
        ['label'=>'Finalized',     'count'=>$wf['finalized'] ?? 0,          'desc'=>'Admin approved & finalized',     'color'=>'emerald',
        ...
        ['label'=>'Unscheduled',   'count'=>$schedulingAnalytics['subjects_unscheduled'] ?? 0, 'desc'=>'Subjects without any schedule', 'color'=>'orange',
        ...
        ['label'=>'Conflicts',     'count'=>$wf['conflict_count'] ?? 0,     'desc'=>'Active scheduling conflicts',    'color'=>'rose',
        ...
        <div class="grid grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <div class="card-lift flex flex-col gap-1.5 p-3 rounded-xl border {{ $card['border'] }} {{ $card['glow'] }} transition-all duration-500 cursor-default">
                <div class="flex items-center gap-1.5">
                    ...
                    <span class="text-[10px] font-semibold uppercase tracking-[0.15em] text-slate-500 dark:text-slate-400 truncate">{{ $card['label'] }}</span>
                </div>
                <span class="text-[26px] font-bold tabular-nums text-slate-900 dark:text-white leading-none">{{ number_format($card['count']) }}</span>
    ...
    <div class="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-2xl p-4 shadow-sm dark:shadow-none">
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-4">
                ...
                <span class="text-[12px] text-slate-500">{{ $systemStatus['current_semester'] ?? '—' }}</span>
            </div>
        </div>
        ...
        <div class="mt-5 pt-4 border-t border-slate-100 dark:border-white/[0.05]">
            <div class="flex items-center justify-between mb-2.5">
                ...
                <span class="text-[13px] font-bold text-emerald-500">
                    {{ $wf['finalized'] ?? $schedulingAnalytics['finalized'] ?? 0 }} / {{ $stats['total_subjects'] ?? $stats['total_schedules'] }} finalized
                </span>
            </div>
    ...
    <div class="col-span-12 lg:col-span-5 flex flex-col gap-4">
            ...
            <div class="space-y-5">
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-2">
                            <span class="px-1.5 py-0.5 rounded text-[11px] font-bold border {{ $dc['bg'] }} {{ $dc['text'] }}">{{ $dept['code'] }}</span>
        ...
        <div class="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-2xl p-5 shadow-sm dark:shadow-none flex-1 flex flex-col min-h-0">
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('manage-users') }}" class="text-[11px] font-bold uppercase text-blue-600 dark:text-blue-400 hover:underline">Manage Users</a>
            </div>
            @php
                $auditColors = [
        ...
        <div class="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-2xl p-5 shadow-sm dark:shadow-none">
            ...
            @php
                $statusRows = [
                    ...
                    ['label'=>'Semester Live',   'status'=>$systemReady ? 'healthy' : 'warning',                   'detail'=>$systemReady ? 'Ready' : 'Setup needed'],
                ];
                $dot = [
...
{{-- ═══════════════════════════════════════════════════
     CONFIRMATION MODAL — Mark as Ready
═══════════════════════════════════════════════════════ --}}
@if($confirmingMarkReady)
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop bg-black/40 dark:bg-black/60">
    <div class="relative w-full max-w-md bg-white dark:bg-[#0f172a] rounded-2xl shadow-2xl
                border border-slate-200 dark:border-white/10
                ring-1 ring-emerald-400/30 dark:ring-emerald-500/20
                overflow-hidden banner-slide">

        {{-- Green top accent --}}
        <div class="h-1 w-full bg-gradient-to-r from-emerald-400 via-teal-400 to-emerald-400"></div>

        <div class="p-6">
            {{-- Icon + title --}}
            <div class="flex items-center gap-3 mb-4">
                <div class="w-11 h-11 rounded-xl bg-emerald-100 dark:bg-emerald-500/15
                ...
                <div>
                    <h3 class="text-[16px] font-bold text-slate-900 dark:text-white tracking-tight">Mark System as Ready?</h3>
                    <p class="text-[12px] text-slate-500 mt-0.5">{{ $systemStatus['current_semester'] ?? 'Current semester' }}</p>
                </div>
            </div>

            {{-- Body --}}
            <p class="text-[13px] text-slate-600 dark:text-slate-400 leading-relaxed mb-4">
                This will notify <strong class="text-slate-800 dark:text-slate-200">Dean, OIC, and Assistant Dean</strong> roles that the semester is live.
                They will gain access to the Master Grid and room view immediately.
            </p>

            {{-- Checklist recap --}}
            <div class="space-y-2 mb-5 p-3 rounded-xl bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10">
            ...
            <div class="flex gap-2">
                <button wire:click="cancelMarkReady"
                        class="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-white/10
                               text-slate-600 dark:text-slate-400 text-[13px] font-bold uppercase tracking-wider
                ...
                <button wire:click="confirmMarkReady"
                        class="flex-1 px-4 py-2.5 rounded-xl
                               bg-emerald-500 hover:bg-emerald-600
                               text-white text-[13px] font-bold uppercase tracking-wider
...
{{-- ═══════════════════════════════════════════════════
     CONFIRMATION MODAL — Revert to Not Ready
═══════════════════════════════════════════════════════ --}}
@if($confirmingMarkNotReady)
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop bg-black/40 dark:bg-black/60">
    <div class="relative w-full max-w-md bg-white dark:bg-[#0f172a] rounded-2xl shadow-2xl
                border border-slate-200 dark:border-white/10
                ring-1 ring-rose-400/30 dark:ring-rose-500/20
                overflow-hidden banner-slide">

        {{-- Rose top accent --}}
        <div class="h-1 w-full bg-gradient-to-r from-rose-400 via-orange-400 to-rose-400"></div>

        <div class="p-6">
            {{-- Icon + title --}}
            <div class="flex items-center gap-3 mb-4">
                <div class="w-11 h-11 rounded-xl bg-rose-100 dark:bg-rose-500/15
                ...
                <div>
                    <h3 class="text-[16px] font-bold text-slate-900 dark:text-white tracking-tight">Revert to Not Ready?</h3>
                    <p class="text-[12px] text-slate-500 mt-0.5">{{ $systemStatus['current_semester'] ?? 'Current semester' }}</p>
                </div>
            </div>

            {{-- Body --}}
            <p class="text-[13px] text-slate-600 dark:text-slate-400 leading-relaxed mb-5">
                This will hide the <strong class="text-slate-800 dark:text-slate-200">Master Grid and room view</strong> from Dean, OIC, and Assistant Dean roles immediately.
                They will see the "Waiting on Config" holding screen again until you mark the system as ready once more.
            </p>
