    ...
    <tr class="divide-x divide-slate-300">
        ...
        @foreach($days as $day)
            @php 
                ...
                $times = explode(' - ', $slot);
                $currentStart = \Carbon\Carbon::createFromFormat('h:i A', trim($times[0]))->format('H:i:s');
                $currentEnd = \Carbon\Carbon::createFromFormat('h:i A', trim($times[1] ?? ''))->format('H:i:s');
            ...
            <td class="p-1 h-24 relative transition-all group"
                ...
                @else
                    {{-- Empty Slot: pointer-events-none prevents the '+' icon from blocking the drop --}}
                    <div class="pointer-events-none h-full w-full border border-dashed border-slate-300 rounded-xl flex items-center justify-center text-slate-300 text-xl font-black group-hover:border-blue-400 group-hover:text-blue-400 transition-colors">
                        +
                    </div>
                @endif
            </td>
